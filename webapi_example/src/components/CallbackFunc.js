// 자바스크립트는 비동기적으로 동작합니다.
// 그래서 먼저 실행된 작업이 끝나지 않았더라도 다음 작업이 시작될 수 있습니다.
// 콜백 함수를 이용하면 특정 코드에 순서를 정해서, 
// 원하는 시점에 실행 할 수 있습니다. 
...
class CallbackFunc extends Component {
    ...
    logPrint(param, callback) {
        ...
        param += param
        // 여기서 실행된 callback 함수는 앞서, 본 소스의 10행의 function(return1){ 에서부터
        // 18행의 }.bind(this)) 까지 입니다. 
        // 15행에서 this.logPrint(return1, function(return2) { 로 
        // 다시 한번 logPrint 함수를 실행하는데 앞서 10행의 첫 번째 함수 실행에서 반환된
        // return1 값(2)을 파라미터로 전달합니다. 
        callback(param);
        // 10행의 this와 15행의 this는 다릅니다.
        // 그래서, this로 logPrint 함수에 접근해 사용하려고 하면 에러가 발생합니다.
        // 함수 밖의 this를 함수 안에서도 동일하게 사용허기 위해서
